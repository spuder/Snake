...
public class Game
{
	...
	public void checkHasCollidedWithSnakeBody(Snake aSnake)
	{
		...
		//Iterates through all the bodies of all the snakes and sees it if it has gone inside (collided) with another snake
	 ...
	 * Checks isSolidWalls==true to see if wall collisions kill the snake
	 * If the snake attempts to go past the wall and isSolidWalls = true; the snake is dead
	 * If isSolidWalls = false this method always returns false since the snake can 
 ...
 public boolean hasCollidedWithWall()
 {
	...
	public boolean checkTimeout(model.Snake aSnake)
	{
		...
		{
			logger.debug("We have waited for " + snakeSpeed +" milliseconds, it is time to advance snake "+ aSnake.getColor() );
