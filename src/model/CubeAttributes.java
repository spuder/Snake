...
public class CubeAttributes {
	...
	// In this case we do want an instance of CubeAttribute, but only 1
	// instance.
	...
	public static void setxNumberOfLedsPerRow(int xNumberOfLedsPerRow) {
		if (xNumberOfLedsPerRow < 1)
			throw new IllegalStateException(
					"CubeAttributes.xNumberOfLedsPerRow must contain atleast 1, recived: "
							+ xNumberOfLedsPerRow);
	...
	public static void setyNumberOfRowsPerPanel(int yNumberOfRowsPerPanel) {
		if (yNumberOfRowsPerPanel < 1)
			throw new IllegalStateException(
					"CubeAttributes.yNumberOfRowsPerPanel must contain atleast 1, recived: "
							+ yNumberOfRowsPerPanel);
	...
	public static void setzNumberOfPanelsPerCube(int zNumberOfPanelsPerCube) {
		// This is different than X or Y because you can have a cube with only 1
		// Panel
		if (zNumberOfPanelsPerCube < 0)
			throw new IllegalStateException(
					"CubeAttributes.zNumberOfPanelsPerCube cannot be negative, zNumberOfPanelsPerCube = "
							+ zNumberOfPanelsPerCube);
	...
	public static int getNumberOfLedsPerCube() {
		...
		if (xNumberOfLedsPerRow < 1)
			throw new IllegalStateException(
					"CubeAttributes.xNumberOfLedsPerRow in method getNumberOfLedsPerCube has a value of: "
							+ xNumberOfLedsPerRow
							+ "\nYou must set a value greater than 1");
		// User is attempting to getNumber of Leds without having set X >= 1
		if (yNumberOfRowsPerPanel < 1)
			throw new IllegalStateException(
					"CubeAttributes.yNumberOfRowsPerPanel in method getNumberOfLedsPerCube has a value of: "
							+ yNumberOfRowsPerPanel
							+ "\nYou must set a value greater than 1");
		// Unlike x and y, z CAN be 0. It would be a single dimensional plane,
		// and not a cube
		if (zNumberOfPanelsPerCube < 0)
			throw new IllegalStateException(
					"CubeAttributes.zNumberOfPanelsPerCube in method getNumberOfLedsPerCube has a negative value of: "
							+ zNumberOfPanelsPerCube);
